 ...
 * @author PC_HONGQUAN
 */
@WebServlet(name = "CreateAccountByAdminServlet", urlPatterns = {"/createaccountbyadmin"})
public class CreateAccountByAdminServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = siteMaps.getProperty(MyAppConstants.AdminFeatures.LOAD_CREATE_ACCOUNT_ACTION);
        boolean error = false;
        ...
        try {
            ...
            if (!Util.isValidEmail(email)) {
                e.setEmailFormatError("Email field must be in a valid format");
                error = true;
            }
            if (fullname.isEmpty()) {
                e.setFullnameIsEmptyError("Full name  cannot be left empty.");
                error = true;
            ...
            AccountDTO account = new AccountDTO(email, password, img, fullname,
                    address, phonenum, false, new RoleDTO(Integer.parseInt(roleId)));
            AccountDAO dao = new AccountDAO();
            row = dao.insertAccount(account);
            if (row != 0) {
                url = siteMaps.getProperty(MyAppConstants.AdminFeatures.MANAGE_USER_ACTION);

            }

        } catch (ClassNotFoundException ex) {
            log("CreateAccountByAdminServlet ClassNotFound " + ex.getMessage());
        } catch (NamingException ex) {
            log("CreateAccountByAdminServlet NamingException " + ex.getMessage());
        } catch (SQLException ex) {
            log("CreateAccountByAdminServlet SQLException " + ex.getMessage());
            e.setAccountIsExisted("Account is existed");
            request.setAttribute("INSERTERR", e);
        } finally {
