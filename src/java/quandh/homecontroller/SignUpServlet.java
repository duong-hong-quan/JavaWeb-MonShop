...
public class SignUpServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = siteMaps.getProperty(MyAppConstants.LoginAndLogoutFeatures.SIGN_UP_JSP);
        int row = 0;
        boolean error = false;
        ...
        try {
            ...
            if (!password.equals(repassword)) {
                e.setPasswordNotMatch("Repeat password field must match the password field.");
                error = true;
            ...
            if (error == false) {
                ...
                AccountDTO acc = new AccountDTO(email, password, fullname, address, phonenum, new RoleDTO(2));
                row = dao.insertAccount(acc);
                if (row != 0) {
                    url = siteMaps.getProperty(MyAppConstants.LoginAndLogoutFeatures.LOGINPAGE);
                }
            }
        } catch (NamingException ex) {
            log("SignUpServlet NamingException " + ex.getMessage());
        } catch (ClassNotFoundException ex) {
            log("SignUpServlet ClassNotFound " + ex.getMessage());
        } catch (SQLException ex) {
            log("SignUpServlet SQLException " + ex.getMessage());
